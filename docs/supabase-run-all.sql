-- Caret: complete database setup, safe to run repeatedly.
-- Combines waitlist + analytics + applications + marketing setup; every
-- statement is idempotent (if-not-exists / drop-and-recreate policies),
-- so re-running never errors and never duplicates data.
...
drop policy if exists "anon can apply" on public.tester_applications;
create policy "anon can apply"
  on public.tester_applications for insert to anon with check (true);

-- ── 4) Outreach targets (service-role only: RLS on, no anon policies) ──
...
create unique index if not exists outreach_targets_name_key on public.outreach_targets (name);

-- ── 5) Reddit posts (service-role only) ────────────────────────
...
-- ── 6) Seed outreach targets (skips any name that already exists) ──
-- Full research + email bodies: docs/marketing/2026-07-15-influencer-targets-and-specific-drafts.md
